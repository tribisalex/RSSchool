...
const warmUp = birdsData[0];
const forestBirds = birdsData[1];
const sparrows = birdsData[2];
const songbirds = birdsData[3];
const birdsOfPrey = birdsData[4];
const seabirds = birdsData[5];
...
function playMistake() {
  var audio = new Audio('../assets/sounds/mistake.mp3');
  audio.play();
}
...
const playQuiz = (birdsData, randomNum) => {
  ...
  console.log('activeBird', activeBird);
  const audioSrc = activeBird.audio;
...
//     if (audio.muted) {
//       audioPlayer.querySelector(".volume-button").classList.add('mute');
//     } else {
//       audioPlayer.querySelector(".volume-button").classList.remove('mute');
//     }
//   });
//
...
function newCathegory(index) {
  countClick = 6;
  scoreCount = 0;
  score.textContent = `Score: ${scoreCount}`;
  nextLevel.setAttribute('disabled', true);
  playQuiz(birdsData[index], getRandomNum());
  questionName.textContent = '*****';
  question__image.src = '../assets/images/plug.jpg';
  isSelectedWin = false;
  isSelectedMistake = false;
