...
const categoryRu = ['Разминка', 'Воробьиные', 'Лесные птицы', 'Певчие птицы', 'Хищные птицы', 'Морские птицы'];
const categoryEn = ['Warm-up', 'Sparrows', 'Forest birds', 'Songbirds', 'Birds of Prey', 'Seabirds'];

const questionName = document.querySelector('.question__description-name');
const answerItem = document.querySelectorAll('.answer-block__options-item__name');
...
const headerBlock = document.querySelector('.nav');
const resultBlock = document.querySelector('.result-quiz');
const resultScore = document.querySelector('.result-quiz__score');
const nextLevelBlock = document.querySelector('.next-level');
const answerIntroBlock = document.querySelector('.answer-block__content-intro');
const answerPlugText = document.querySelector('.answer-block__content__plug');
const resultText = document.querySelector('.result-quiz__h1');
const resultName = document.querySelector('.result-quiz__name');
const startQuizButton = document.querySelector('.start-quiz__button');

const answerName = document.querySelector('.answer-block__content-name');
const answerNameLatin = document.querySelector('.answer-block__content-name-latin');
const answerImage = document.querySelector('.answer-block__description__image-block__img');
const answerText = document.querySelector('.answer-block__content-text');
const question__image = document.querySelector('.question__image-block__img');
const question__name = document.querySelector('.question__description-name');
const nextLevel = document.querySelector('.next-level__button');

// change lang
const langSelect = document.querySelector('.select-lang');

let currentLang = localStorage.getItem('lang') === null ? 'Ru' : localStorage.getItem('lang');
langSelect.value = localStorage.getItem('lang') === null ? 'Ru' : localStorage.getItem('lang');
answerPlugText.textContent = currentLang === 'Ru' ? 'Прослушайте плеер и выберите вариант ответа... ' : 'Listen to the player and choose the answer option...';
nextLevel.textContent = currentLang === 'Ru' ? 'Далее' : 'Next level';
resultText.textContent = currentLang === 'Ru' ? 'Поздравляем с прохождением викторины' : 'Congratulations on passing the quiz';
resultName.textContent = currentLang === 'Ru' ? 'Ваш результат: ' : 'Your result:';
startQuizButton.textContent = currentLang === 'Ru' ? 'Начать викторину' : 'Start quiz';

langSelect.addEventListener('change', () => {
  localStorage.setItem('lang', langSelect.value);
  currentLang = localStorage.getItem('lang');
  birdsData = currentLang === 'Ru' ? birdsDataRu : birdsDataEn;
  if (!isSelectedWin) playQuiz(birdsData[countLevel], getRandomNum());
  changeCategory();
  resultName.textContent = currentLang === 'Ru' ? 'Ваш результат: ' : 'Your result:';
  startQuizButton.textContent = currentLang === 'Ru' ? 'Начать викторину' : 'Start quiz';
  resultText.textContent = currentLang === 'Ru' ? 'Поздравляем с прохождением викторины' : 'Congratulations on passing the quiz';
  answerPlugText.textContent = currentLang === 'Ru' ? 'Прослушайте плеер и выберите вариант ответа... ' : 'Listen to the player and choose the answer option...';
  nextLevel.textContent = currentLang === 'Ru' ? 'Далее' : 'Next level';
  score.textContent = currentLang === 'Ru' ? `Очки: ${scoreCount}` : `Score: ${scoreCount}`;
});

let birdsData = currentLang === 'Ru' ? birdsDataRu : birdsDataEn;

let activeBirds = birdsData[0];
let activeBird;
let random;
let selectedBird;
let scoreCount = 0;
let countClick = 6;
let countLevel = 0;
let isPlayQuestion = false;
let audioSrcQuestion;

score.textContent = `Score: ${scoreCount}`;

// sound mistake
function playMistake() {
  let audio = new Audio('../assets/sounds/mistake.mp3');
  audio.play();
}

// sound done
function playDone() {
  let audio = new Audio('../assets/sounds/done.mp3');
  audio.play();
}
...
const playQuestion = document.querySelector('.play');
const playAnswer = document.querySelector('.play__answer');
const timeline = document.querySelectorAll(".timeline");
const volumeSlider = document.querySelectorAll(".volume-slider");
const volumePer = document.querySelectorAll(".volume-percentage");
const progressBar = document.querySelectorAll(".progress");
const volumeBut = document.querySelectorAll(".volume-button");

const playQuiz = (birdsData, randomNum) => {
  activeBird = birdsData[randomNum];
  activeBirds = birdsData;
  random = randomNum;
  console.log(activeBird.name);
  audioSrcQuestion = activeBird.audio;
  ...
  shuffleArr(birdsData);

  answerItem.forEach((item, index) => {
    item.textContent = birdsData[index].name;
  })

  audioPlayer(audioSrcQuestion, audioPlayerQuestion, audioQuestion, playQuestion, timeline[0], volumeSlider[0], volumePer[0], progressBar[0], volumeBut[0]);
}

function audioPlayer(src, audioPlayer, audio, play, time, volume, volumePer, progressBar, volumeBut) {
  audio.src = src;

  audio.addEventListener(
    "loadeddata",
    () => {
      audioPlayer.querySelector(".music-time .length").textContent = getTimeCodeFromNum(
        audio.duration
      );
      audio.volume = .75;
  ...
  time.addEventListener("click", e => {
    const timelineWidth = window.getComputedStyle(time).width;
  ...
  volume.addEventListener('click', e => {
    ...
    volumePer.style.width = newVolume * 100 + '%';
  }, false)

  setInterval(() => {
    progressBar.style.width = audio.currentTime / audio.duration * 100 + "%";
    audioPlayer.querySelector(".music-time .current").textContent = getTimeCodeFromNum(
      audio.currentTime
    );
  }, 500);

  volumeBut.addEventListener("click", () => {
    const volumeEl = audioPlayer.querySelector(".volume");
    audio.muted = !audio.muted;
    if (audio.muted) {
      volumeBut.classList.add('mute');
    } else {
      volumeBut.classList.remove('mute');
    }
  });
...
function getRandomNum() {
  return Math.floor(Math.random() * (5)); //Максимум и минимум включаются
}

playQuiz(birdsData[0], getRandomNum());

const cathegoryBirds = document.querySelectorAll('.nav__ul-item');
let activeCathegoryBirds = document.querySelector('.nav__ul-item.active') || cathegoryBirds[0];

function changeCategory() {
  let category = currentLang === 'Ru' ? categoryRu : categoryEn;
  cathegoryBirds.forEach((item, index) => {
...
const answerBlockOptions = document.querySelectorAll('.answer-block__options-item');
let selectedAnswer = 0;
let selectedAnswerCircle;

nextLevel.setAttribute('disabled', true);
question__name.textContent = '*****'
...
nextLevel.addEventListener('click', () => {
  ...
  if (countLevel >= 6) {
    ...
    for (let i = 0; i < cathegoryBirds.length; i++) {
      if (i === countLevel) {
        activeCathegoryBirds.classList.remove('active');
        cathegoryBirds[countLevel].classList.add('active');
        activeCathegoryBirds = cathegoryBirds[countLevel];
      }
    }

    if (playQuestion.classList.contains('pause')) {
      playQuestion.classList.remove('pause');
    }

    hideAnswerDescription();
    playQuiz(birdsData[countLevel], random);
    isSelectedWin = false;
    isSelectedMistake = false;
    questionName.textContent = '*****';
    question__image.src = '../assets/images/plug.jpg';
    removeCircleStyle();
    nextLevel.setAttribute('disabled', true);
    countClick = 6;
  }
  ...
  item.addEventListener('click', () => {
    ...
    for (let i = 0; i <= activeBirds.length - 1; i++) {
      if (selectedAnswer === activeBirds[i].name) {
        ...
        answerName.textContent = selectedBird.name;
        answerNameLatin.textContent = selectedBird.species;
        answerImage.src = selectedBird.image;
        answerText.textContent = selectedBird.description;
        const audioSrcAnswer = selectedBird.audio;
      ...
      if (selectedAnswer === activeBird.name) {
        if (!isSelectedWin) {
          selectedWin();
        }
      } else {
        if (!isSelectedMistake) {
          selectedMistake();
        }
      }
    }
...
let isSelectedWin = false;
let isSelectedMistake = false;

function selectedWin() {
  question__image.src = activeBird.image;
  playDone();
  question__name.textContent = activeBird.name;
  selectedAnswerCircle.classList.add('active');
  scoreCount = scoreCount + countClick;
  score.textContent = `Score: ${scoreCount}`;
  // selectedWin = 0;
  // selectedMistake = 0;
  isSelectedWin = true;
...
function selectedMistake() {
  playMistake();
  selectedAnswerCircle.classList.add('mistake');
}

function removeCircleStyle() {
  for (let i = 0; i <= selectedAnswersCircle.length - 1; i++) {
    selectedAnswersCircle[i].classList.contains('active') ? selectedAnswersCircle[i].classList.remove('active') : null;
    selectedAnswersCircle[i].classList.contains('mistake') ? selectedAnswersCircle[i].classList.remove('mistake') : null;
  }
}
...
function showResult() {
  ...
  nextLevelBlock.classList.add('hide');
  resultScore.textContent = scoreCount;
  score.classList.add('hide');
}

function showAnswerDescription() {
  answerIntroBlock.classList.remove('hide');
  answerText.classList.remove('hide');
  answerPlugText.classList.add('hide');
}

function hideAnswerDescription() {
  answerIntroBlock.classList.add('hide');
  answerText.classList.add('hide');
  answerPlugText.classList.remove('hide');
}