...
const galleryOptions = document.querySelector('.gallery__popap-list__options');
const playGallery = document.querySelector('.play__gallery');
const galleryImage = document.querySelector('.gallery__popap-item__image');
const galleryName = document.querySelector('.gallery__popap-item-name');
const galleryNameLatin = document.querySelector('.gallery__popap-item-name-latin');
const galleryText = document.querySelector('.gallery__popap-text');
...
document.onclick = e => {
  if (e.target.id === 'overlay' || e.target.id === 'close-popap' || e.target.id === 'overlay-index') {
    ...
    galleryPopap.classList.remove('active');
  } else if (e.target.id === 'gallery-text') {
    const galleryListItem = document.querySelectorAll('.gallery__popap-list__options-item');
    galleryListItem.forEach((item) => {
      item.addEventListener('click', () => {
        for (let i = 0; i <= birdsDataGallery.length - 1; i++) {
          if (birdsDataGallery[i].name === item.textContent) {
            galleryImage.src = birdsDataGallery[i].image;
    ...
    galleryPopap.classList.add('active');
...
for (let i = 0; i <= birdsDataGallery.length - 1; i++) {
  ...
  li.setAttribute('class', 'gallery__popap-list__options-item');
  li.textContent = birdsDataGallery[i].name;
  galleryOptions.appendChild(li);
}
